...
async function fetchExistingEpisodes(_targetNpub: string, episodeAuthorPubkey: string): Promise<NostrEvent[]> {
  ...
  console.log(`📋 Episode author pubkey: ${episodeAuthorPubkey.substring(0, 8)}...`);
  ...
  try {
    const events = await queryRelay(relayUrl, {
      kinds: [30054],
      authors: [episodeAuthorPubkey], // Filter by episode author pubkey
      limit: 200,
    });
...
async function main() {
  ...
  try {
    ...
    let targetPubkey = '';
    try {
      const decoded = nip19.decode(config.targetNpub);
      if (decoded.type === 'npub') {
        targetPubkey = decoded.data;
      }
    } catch (error) {
      console.warn('⚠️  Failed to decode target npub:', error);
    }
